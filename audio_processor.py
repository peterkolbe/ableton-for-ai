...
def _process_spectrogram(magnitude, sr, hop_length, audio_path, output_path):
    ...
    with matplotlib_lock:
        ...
        fig.savefig(
            ...
            dpi=120,
            bbox_inches="tight",
            pil_kwargs={"quality": config.SPECTROGRAM_QUALITY, "optimize": True},
        )
...
def _process_band_energy(
    ...
    if y is not None:
        ...
        try:
            ...
            if len(block_lufs_mom) > 0:
                ...
                lufs_mom_frames = np.interp(
                    times,
                    block_times_mom,
                    block_lufs_mom,
                    left=block_lufs_mom[0],
                    right=block_lufs_mom[-1],
                )
            ...
            if len(block_lufs_st) > 0:
                ...
                lufs_st_frames = np.interp(
                    times,
                    block_times_st,
                    block_lufs_st,
                    left=block_lufs_st[0],
                    right=block_lufs_st[-1],
                )
        except Exception:
            pass
    ...
    for t_idx, t in enumerate(times):
        if summary_only:
            ...
            row = {
                ...
                "e": [
                    round(
                        float(10 * np.log10(max(power[masks[name], t_idx].sum(), 1e-12))),
                        1,
                    )
                    for name in bands
                ],
    ...
    transients = []
    if y is not None:
        y_mono_for_transients = librosa.to_mono(y) if y.ndim > 1 else y
        transients = _detect_transients(y_mono_for_transients, sr)
    ...
    if summary_only:
        ...
        if len(json_str) > limit_bytes and len(frames) > 0:
            ...
            log_info(
                f"Summary size ({len(json_str) // 1024}KB) exceeds limit ({config.SUMMARY_MAX_SIZE_KB}KB). Splitting into multiple chunks."
            )
...
def process_audio_file(file_path: str, output_dir: str, summary_only: bool = False) -> bool:
    ...
    try:
        ...
        # Stereo Correlation
        corr = 1.0
        if y.ndim > 1 and y.shape[0] == 2 and np.max(np.abs(y)) > 1e-8:
